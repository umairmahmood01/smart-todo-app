...
data class HomeUiState(
    ...
    val isEmpty: Boolean = !isLoading && feedItems.isEmpty()

    /** True when the list is empty only because of the current filters. */
    val isFilteredEmpty: Boolean = isEmpty && hasActiveFilters

    /**
     * Categories worth rendering as chips: everything that currently has tasks or lists,
     * plus anything the user has selected (so a selected chip never disappears mid-filter).
     */
